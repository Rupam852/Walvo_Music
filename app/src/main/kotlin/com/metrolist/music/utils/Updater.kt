...
object Updater {
    ...
    fun compareVersions(v1: String, v2: String): Int {
        val cleanV1 = v1.replace(Regex("[^0-9.]"), "").trim('.')
        val cleanV2 = v2.replace(Regex("[^0-9.]"), "").trim('.')
        ...
        for (i in 0 until assetsArray.length()) {
            ...
            val (arch, variant) = when {
                ...
                name.startsWith("app-") && name.endsWith("-release.apk") -> {
                    val parsedArch = name.removePrefix("app-").removeSuffix("-release.apk")
                    (if (parsedArch == "foss") "universal" else parsedArch) to "foss"
                }
                name.startsWith("app-") && name.endsWith("-with-Google-Cast.apk") -> {
                    val parsedArch = name.removePrefix("app-").removeSuffix("-with-Google-Cast.apk")
                    (if (parsedArch == "gms") "universal" else parsedArch) to "gms"
                }
